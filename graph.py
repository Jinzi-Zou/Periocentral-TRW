...
class NeighborFinder:
    def __init__(self,data_name,time_windows, adj_list, bias=0,beta =0,gama =0, ts_precision=PRECISION, use_cache=False, sample_method='multinomial',full=False):
        """
        Params
        ------
        node_idx_l: List[int]
        ...
        self.beta = beta
        self.gama = gama
        node_idx_l, node_ts_l, edge_idx_l, off_set_l, time_prob_l,self.nodeedge2idx = self.init_off_set(adj_list)
        print("initial off set")
        self.time_prob_l = time_prob_l
        ...
        if full:
            file = f'./centrality_prob/{self.data_name}_{self.gama}_full.npy'
        else:
            file = f'./centrality_prob/{self.data_name}_{self.gama}_partial.npy'
        if not os.path.exists(file):
            centrality_prob_l = self.init_centrality_prob(adj_list,full)
        else:
            centrality_prob_l = np.load(file)


        print("centrality_prob_l")
        self.centrality_prob_l = centrality_prob_l
    ...
    def init_off_set(self, adj_list):
        ...
        for i in range(len(adj_list)):
            ...
            n_ts_l.extend(ts_l)
            time_prob_l.append(self.compute_binary_prob(np.array(ts_l)))
        ...
        time_prob_l = np.concatenate(time_prob_l)
        off_set_l = np.array(off_set_l)
        assert(len(n_idx_l) == len(n_ts_l))
        assert(off_set_l[-1] == len(n_ts_l))
        
        return n_idx_l, n_ts_l, e_idx_l, off_set_l,time_prob_l, nodeedge2idx

    def init_centrality_prob(self,adj_list,full):
        cen_prob_l = []
        ...
        # for i, curr in tqdm(enumerate(adj_list)):
            cen_prob = []
            curr = adj_list[i]
            curr = sorted(curr, key=lambda x: x[2])
            # for one_hop_n in tqdm(curr, desc="Inner Loop", leave=False):
            for one_hop_n in curr:
                start = off_set_l[int(one_hop_n[0])]
                end = off_set_l[int(one_hop_n[0]) + 1]
                ...
                if one_hop_n[1] is None:
                    cut_idx = bisect_left_adapt(neighbors_ts,
                                                one_hop_n[2])  # very crucial to exempt ties (so don't use bisect)

                else:
                    # use quick index mapping to get node index and edge index
                    # a problem though may happens when there is a tie of timestamps
                    cut_idx = self.nodeedge2idx[one_hop_n[0]].get(one_hop_n[1]) if one_hop_n[0] > 0 else 0
                    if cut_idx is None:
                        raise IndexError('e_idx {} not found in edge list of {}'.format(one_hop_n[1], one_hop_n[0]))
                ngh_sec_idx, ngh_sec_eidx, ngh_sec_ts = (neighbors_idx[:cut_idx], neighbors_e_idx[:cut_idx], neighbors_ts[:cut_idx])
                centrality_prob_l = self.compute_centrality_prob(ngh_sec_idx,ngh_sec_eidx,ngh_sec_ts)
                cen_prob.append(centrality_prob_l)
            # cen_prob /= np.cumsum(cen_prob)
            cen_prob_l.extend(cen_prob)

        cen_prob_l = np.concatenate(cen_prob_l)
        if full:
            np.save(f'./centrality_prob/{self.data_name}_{self.gama}_full.npy',cen_prob_l)
        else:
            np.save(f'./centrality_prob/{self.data_name}_{self.gama}_partial.npy', cen_prob_l)
        return cen_prob_l
    ...
    def compute_frequency_prob(self, nei_before,ts_l):
        if len(ts_l) == 0:
            return np.array([])
        count_li = Counter(nei_before)
        count_sort = sorted(count_li.items(),key=operator.itemgetter(1),reverse=False)
        num=[value for (key,value) in count_sort]
        node = [key for (key,value) in count_sort]
        exp_fre_l = np.exp(-self.beta /np.array(num))/np.sum(np.exp(-self.beta /np.array(num)))
        # exp_fre_l /= np.cumsum(exp_fre_l)
        prob_fre = dict(zip(node,exp_fre_l))
        #         print( exp_ts_l_cumsum, exp_ts_l, ts_l, exp_ts_l)
        return prob_fre

    def compute_centrality_prob(self, ngh_sec_idx, ngh_sec_eidx, ngh_sec_ts):
        uniq_time = len(np.unique(ngh_sec_ts))
        if uniq_time > 1:
            all_combination = math.comb(len(ngh_sec_ts), 2)
            re_ngh = defaultdict(list)
            for key, value in zip(ngh_sec_idx, ngh_sec_ts):
                re_ngh[key].append(value)

            # 使用列表推导式计算 sum_re_ngh
            sum_re_ngh = sum(math.comb(len(count), 2) for count in re_ngh.values() if len(count) > 1)

            re_time = defaultdict(list)
            for key, value in zip(ngh_sec_ts, ngh_sec_idx):
                if value not in re_time[key]:
                    re_time[key].append(value)

            # 使用列表推导式计算 sum_re_time
            sum_re_time = sum(math.comb(len(count), 2) for count in re_time.values() if len(count) > 1)

            temporal_walk = all_combination - sum_re_ngh - sum_re_time
            centrality_prob = np.exp(-self.gama / np.array([temporal_walk])) if temporal_walk!=0 else np.\
                array([0])
        else:
            return np.array([0])

        return centrality_prob
    ...
    def get_ts2idx(self, sorted_triples):
        ...
        for i, (n_idx, e_idx, ts_idx) in enumerate(sorted_triples):
            ...
            last_ts = ts_idx
            last_e_idx = e_idx
        return ts2idx  #按时间顺序对链路重新设置index，发生时间相同的链路有同样的index

    def find_before(self, src_idx, cut_time, e_idx=None, return_binary_prob=False, centrality=False):
        """
        Params
        ------
        src_idx: int
        ...
        edge_idx_l = self.edge_idx_l
        off_set_l = self.off_set_l
        time_prob_l = self.time_prob_l
        centrality_prob_l = self.centrality_prob_l

        # binary_prob_l = self.binary_prob_l  # TODO: make it in preprocessing
        start = off_set_l[int(src_idx)]
        end = off_set_l[int(src_idx + 1)]
        neighbors_idx = node_idx_l[start: end]
        neighbors_ts = node_ts_l[start: end]
        neighbors_e_idx = edge_idx_l[start: end]
        ...
        elif centrality is True:
            centrality_time = [item for item in neighbors_ts if item <= cut_time]
            cut_idx = len(centrality_time)
        ...
        if not return_binary_prob:
            ...
            # neighbors_ts = neighbors_ts[:cut_idx]
            # if len(neighbors_ts) > 1:
            #     diff = np.diff(neighbors_ts)
            #     cumulative_sum = np.cumsum(diff[::-1])
            #     if np.argmax(cumulative_sum > self.time_windows)!=0:
            #         start_index = len(neighbors_ts) - np.argmax(cumulative_sum > self.time_windows) - 1
            #     else:start_index = 0
            # else:
            #     start_index = 0  # or handle the empty case as appropriate for your application
            neighbors_time_prob = time_prob_l[start: end]
            neighbors_cen_prob = centrality_prob_l[start: end]
            result = (
            neighbors_idx[:cut_idx], neighbors_e_idx[:cut_idx], neighbors_ts[:cut_idx],neighbors_time_prob[:cut_idx],
            neighbors_cen_prob[:cut_idx])
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbor=20, e_idx_l=None):
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ngh_idx, ngh_eidx, ngh_ts, ngh_time_prob, ngh_cen_prob= self.find_before(src_idx, cut_time, e_idx=e_idx_l[i] if e_idx_l is not None else None,
            ...
            if ngh_time_prob.size==0:  # self.sample_method is multinomial
                ...
                sampled_idx = seq_binary_sample(ngh_time_prob, ngh_cen_prob, num_neighbor)

                # sampled_node = seq_binary_sample(binary_prob_l_timeinterval, binary_prob_l_fre, binary_prob_l_centrality, num_neighbor)
                # print("sample done")
