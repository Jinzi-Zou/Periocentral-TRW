...
full_ngh_finder = NeighborFinder(DATA,TIME_WINDOWS,full_adj_list, bias=args.bias,beta =args.beta,gama = args.gama, use_cache=NGH_CACHE, sample_method=args.pos_sample) #所有的链路
...
for src, dst, eidx, ts in zip(val_src_l, val_dst_l, val_e_idx_l, val_ts_l):
    partial_adj_list[src].append((dst, eidx, ts))
    partial_adj_list[dst].append((src, eidx, ts))
partial_ngh_finder = NeighborFinder(DATA,TIME_WINDOWS,partial_adj_list, bias=args.bias,beta =args.beta,gama = args.gama, use_cache=NGH_CACHE, sample_method=args.pos_sample)  #训练集和验证集的链路
...
# final testing
best_checkpoint_path = './saved_checkpoints/1710120324.6282341-Flights-t-walk-3-64k1k1-108/checkpoint-epoch-2.pth'
