...
class CAWN(torch.nn.Module):
    def __init__(self, args,device,n_feat, e_feat,TIME_WINDOWS, agg='tree',
        ...
        self.time_dim = pos_dim  # default to be time feature dimension
        ...
        self.affinity_score = MergeLayer(args.tree_h_size, args.tree_h_size, args.tree_h_size, 1, non_linear=not self.walk_linear_out) #torch.nn.Bilinear(self.feat_dim, self.feat_dim, 1, bias=True)
        # self.affinity_score = MergeLayer(self.feat_dim, self.feat_dim, self.feat_dim, 1,  # num_class,  # MC
        #                                  non_linear=not self.walk_linear_out)
        ...
        #link predicor
        self.linkpredictor=self.init_predicor(args)
        self.treeLSTM = self.init_treeLSTM(args)

        self.time_windows = TIME_WINDOWS

    def init_predicor(self,args):
        linkpredictor = LinkPredictor(args.tree_h_size+self.feat_dim, args.link_out_hidden, 1,
                              args.link_n_layers, args.link_dropout)
        return linkpredictor
    def init_treeLSTM(self,args):
        model = TreeLSTM(
            args.bs*(1+args.n_layer*int(args.n_degree[0])),
            self.pos_dim,
            args.tree_h_size,
            args.tree_dropout,
            cell_type="childsum",
    ...
    def contrast_original(self, src_idx_l, tgt_idx_l, bgd_idx_l, cut_time_l, e_idx_l=None, test=False):
        ...
        pos_score = self.forward_caw(src_idx_l, tgt_idx_l, cut_time_l, (subgraph_src, subgraph_tgt), test=test)
        self.flag_for_cur_edge = False
        neg_score1 = self.forward_caw(src_idx_l, bgd_idx_l, cut_time_l, (subgraph_src, subgraph_bgd), test=test)
    ...
    def forward_caw(self, src_idx_l, tgt_idx_l, cut_time_l, subgraphs=None, test=False):
        ...
        src_embed = self.forward_msg_caw(src_idx_l, cut_time_l, subgraph_src, test=test)
        tgt_embed = self.forward_msg_caw(tgt_idx_l, cut_time_l, subgraph_tgt, test=test)
    ...
    def contrast(self, args,src_idx_l, tgt_idx_l, bgd_idx_l, cut_time_l, e_idx_l=None, test=False):
        '''
        1. grab subgraph for src, tgt, bgd
        2. add positional encoding for src & tgt nodes
        3. forward propagate to get src embeddings and tgt embeddings (and finally pos_score (shape: [batch, ]))
        ...
        if self.verbosity > 1:
            self.logger.info('grab subgraph for the minibatch, time eclipsed: {} seconds'.format(str(end-start)))
        self.flag_for_cur_edge = True
        pos_src_embed,pos_tgt_embed = self.forward(args,src_idx_l, tgt_idx_l, cut_time_l, (subgraph_src, subgraph_tgt), test=test)
        self.flag_for_cur_edge = False
        neg_src_emb,neg_tgt_embed = self.forward(args, src_idx_l, bgd_idx_l, cut_time_l, (subgraph_src, subgraph_bgd),
                                                  test=test)
        sample_time = end-start
        return pos_src_embed,pos_tgt_embed, neg_src_emb,neg_tgt_embed,sample_time

    def forward(self, args,src_idx_l, tgt_idx_l, cut_time_l, subgraphs=None, test=False):
        if subgraphs is not None:
            subgraph_src, subgraph_tgt = subgraphs
        else: # not used in our code but is still a useful branch when negative sample is not provided
            subgraph_src = self.grab_subgraph(src_idx_l, cut_time_l, e_idx_l=None)  # TODO: self.grab_subgraph(), with e_idx_l
        ...
        if self.agg == 'walk':  #TODO: can we do this later to save position coding time, since walk-based has too much redundancy?
            ...
            subgraph_tgt = self.subgraph_tree2walk(tgt_idx_l, cut_time_l, subgraph_tgt)
        ##用tree_LSTM聚合
        src_embed = self.forward_tree_LSTM(args,src_idx_l, cut_time_l, subgraph_src, test=test)
        tgt_embed = self.forward_tree_LSTM(args,tgt_idx_l, cut_time_l, subgraph_tgt, test=test)
        return src_embed,tgt_embed


    def forward_tree_LSTM(self, args, src_idx_l, cut_time_l, subgraph_src, test=False):
        ...
        hidden_features, masks = self.init_hidden_embeddings(src_idx_l, node_records)  # length self.num_layers+1
        ...
        walk_step = node_records.shape[2]
        walk_sample = node_records.shape[1]
        tree = []

        for idx in range(node_records.shape[0]):
            nid = 0
            sequence = node_records[idx].T.flatten()
            position_features_subgraph = position_features[idx].permute(1, 0, 2).reshape(walk_sample * walk_step,
                                                                                         position_features.shape[-1]).cpu()
            position_features_new = torch.zeros([0, position_features.shape[-1]])
            tree_node = {}
            colum_node = set()
            leaves, level = 0, 0
            for x, i in enumerate(sequence):
                if i not in colum_node:
                    colum_node.add(i)
                    tree_node['-'.join([str(level), str(i)])] = nid
                    nid += 1
                    position_features_new = torch.cat((position_features_new, position_features_subgraph[x].view(1, -1)),
                                                      dim=0)
                    if x >= walk_sample * (walk_step - 1):
                        leaves += 1
                if (x + 1) % walk_sample == 0:
                    level += 1
                    colum_node = set()
            level, u = 0, 0
            v = 0 + walk_sample
            edge = {}
            new_u, new_v = [], []
            # while v<sequence.shape[0]:
            for x, i in enumerate(sequence):

                if (tree_node['-'.join([str(level), str(sequence[x])])],
                    tree_node['-'.join([str(level + 1), str(sequence[x + walk_sample])])]) not in edge:
                    edge[(tree_node['-'.join([str(level), str(sequence[x])])],
                          tree_node['-'.join([str(level + 1), str(sequence[x + walk_sample])])])] = 1
                    new_u.append(tree_node['-'.join([str(level), str(sequence[x])])])
                    new_v.append(tree_node['-'.join([str(level + 1), str(sequence[x + walk_sample])])])
                if x == walk_sample * (walk_step - 1) - 1:
                    break
                if (x + 1) % walk_sample == 0:
                    level += 1
            tree.append(dgl.graph((torch.tensor(new_u), torch.tensor(new_v))))
            tree[idx].ndata["ac"] = position_features_new.cpu()
            mask = torch.zeros(position_features_new.shape[0])
            mask[-leaves:] = 1
            tree[idx].ndata["mask"] = mask.long()
            root_label = torch.zeros(position_features_new.shape[0])
            root_label[0] = 1
            tree[idx].ndata["y"] = root_label.long()
        train_loader = DataLoader(
            dataset=tree,
            batch_size=node_records.shape[0],
        ...
        for step, batch in enumerate(train_loader):
            ...
            self.treeLSTM.pretrain(pretrained_emb=g.ndata["ac"])
            n = g.num_nodes()
            h = torch.zeros((n, args.tree_h_size)).to(self.device)
            c = torch.zeros((n, args.tree_h_size)).to(self.device)
            logits = self.treeLSTM(batch, g, h, c)

        position_embeddings = logits[torch.nonzero(g.ndata['y']).flatten()]
        combined_features = self.random_walk_attn_model.aggregate(hidden_features, time_features, edge_features, position_features)
        combined_features = self.random_walk_attn_model.feature_encoder(combined_features, masks)
        X = self.random_walk_attn_model.projector(combined_features)
        if self.walk_pool == 'sum':
            X = self.random_walk_attn_model.pooler(X, agg='mean')  # we are actually doing mean pooling since sum has numerical issues
        else:
            X = self.random_walk_attn_model.self_attention(X)
            if not self.random_walk_attn_model.mutual:
                X = self.random_walk_attn_model.pooler(X, agg='mean')  # we are actually doing mean pooling since sum has numerical issues
    ...
    def forward_msg_caw(self, src_idx_l, cut_time_l, subgraph_src, test=False):
        node_records, eidx_records, t_records = subgraph_src
        # NOTE: we assume raw node features are static in this problem
        # 1. initialize 0-layer hidden embeddings with raw node features of all hops (later with positional encodings as well)
        # 2. get time encodings for all hops
        ...
        if self.agg == 'tree':
            ...
            for layer in range(n_layer):
                hidden_embeddings = self.forward_msg_layer(hidden_embeddings, time_features[:n_layer + 1 - layer],
                                                           ...
                                                           position_features[:n_layer + 1 - layer],
            ...
            final_node_embeddings = self.forward_msg_walk(hidden_embeddings, time_features, edge_features,
                                                          position_features, masks)
        else:
            raise NotImplementedError('{} forward propagation strategy not implemented.'.format(self.agg))
        return final_node_embeddings

    def forward_msg(self, args,src_idx_l, cut_time_l, subgraph_src, test=False):
        node_records, eidx_records, t_records = subgraph_src
        # NOTE: we assume raw node features are static in this problem
        # 1. initialize 0-layer hidden embeddings with raw node features of all hops (later with positional encodings as well)
        # 2. get time encodings for all hops
        ...
        tree = []
        for idx in range(node_records.shape[0]):
            root=[rix for _ in range(walk_sample)]
            root.extend(list(range(rix+1,rix+1+(walk_step-2)*walk_sample)))
            new_u = torch.tensor(root)
            new_v = torch.tensor(list(range(rix+1,rix+1+(walk_step-1)*walk_sample)))
            tree.append(dgl.graph((new_u, new_v)))
            mask = torch.zeros(1+(walk_step-1)*walk_sample)
            mask[-walk_sample:]=1
            tree[idx].ndata["mask"] = mask.long()
            time_features_new = time_features[idx].permute(1, 0, 2)
            time_features_new = time_features_new.contiguous().view(walk_sample*walk_step,time_features.size(-1))

            time_features_new = torch.cat((time_features_new[0,:].view(1,-1), time_features_new[walk_sample:,:]), 0)
            position_features_new = position_features[idx].permute(1, 0, 2)
            position_features_new = position_features_new.contiguous().view(walk_sample*walk_step,position_features.size(-1))
            position_features_new = torch.cat((position_features_new[0,:].view(1,-1),position_features_new[walk_sample:,:]),0)
            tree[idx].ndata["ac"] = torch.cat((position_features_new,time_features_new),1).cpu()
            root_label = torch.zeros(1+(walk_step-1)*walk_sample)
            root_label[0]=1
            tree[idx].ndata["y"]=root_label
            # rix+=(walk_step-1)*walk_sample
        if self.agg == 'tree':
            n_layer = self.num_layers
            for layer in range(n_layer):
                hidden_embeddings = self.forward_msg_layer(hidden_embeddings, time_features[:n_layer+1-layer],
            ...
            model = TreeLSTM(
                g.num_nodes(),
                tree[0].ndata["ac"].size(-1),
                128,
                0.5,
                cell_type="childsum",
                pretrained_emb=g.ndata["ac"]
            ).to(self.device)

            params_ex_emb = [
                x
                for x in list(model.parameters())
                if x.requires_grad and x.size(0) != g.num_nodes()
            ]
            ...
            for epoch in range(args.epochs):
                ...
                for step, batch in enumerate(train_loader):
                    g = batch.graph.to(self.device)
                    n = g.num_nodes()
                    h = torch.zeros((n, args.h_size)).to(self.device)
                    c = torch.zeros((n, args.h_size)).to(self.device)
                    if step >= 3:
                        t0 = time.time()  # tik

                    logits = model(batch, g, h, c)
            src_embeddings = g.ndata['y']*logits


        else:
            raise NotImplementedError('{} forward propagation strategy not implemented.'.format(self.agg))
        # elif self.agg == 'tree_lstm':


        return final_node_embeddings

    def batcher(self,device):
        def batcher_dev(batch):
            batch_trees = dgl.batch(batch)
            SSTBatch = collections.namedtuple(
                "SSTBatch", ["graph","mask", "label", "a_code"]
            )
            return SSTBatch(
                graph=batch_trees,
                ...
                a_code=batch_trees.ndata["ac"].to(device),
            )
    ...
    def init_hidden_embeddings(self, src_idx_l, node_records):
        ...
        if self.agg == 'tree':
            ...
            hidden_embeddings.append(
                self.node_raw_embed(torch.from_numpy(np.expand_dims(src_idx_l, 1)).long().to(device)))
            ...
            masks = (node_records_th != 0).sum(
                dim=-1).long()  # shape [batch, n_walk], here the masks means differently: it records the valid length of each walk
        else:
            raise NotImplementedError('{} forward propagation strategy not implemented.'.format(self.agg))
    ...
    def retrieve_edge_features(self, eidx_records):
        ...
        if self.agg == 'tree':
            ...
            eidx_records_th[:, :,
            0] = 0  # NOTE: this will NOT be mixed with padded 0's since those paddings are denoted by masks and will be ignored later in lstm
    ...
    def retrieve_position_features(self, src_idx_l, node_records, cut_time_l, t_records, test=False):
        ...
        if self.verbosity > 1:
            self.logger.info(
                'encode positions encodings for the minibatch, time eclipsed: {} seconds'.format(str(end - start)))
